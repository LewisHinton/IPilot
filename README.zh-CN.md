...
这个 GIF 展示 IPilot 的核心闭环：参考图 -> 一致性画像 -> 素材路线 -> 用户确认 -> 提示词与审核交付。重点不是马上出图，而是先把角色身份守住。

![IPilot demo](assets/demo/ipilot-demo.gif)

## 快速开始

安装后，把下面这段直接发给你的 agent：

```text
@ipilot
我已经有一个吉祥物参考图。请把上传的图片作为官方主参考。

目标：为社群聊天创作 6 个表情包。
必须保持：轮廓、配色比例、线条风格、耳机、科技尾巴等身份锚点。
可以变化：姿势、表情、道具和短文案。

请先展示 IP 一致性画像、6 个表情包概念和安全检查。
生成图片前先问我确认。
```
...
| 步骤 | IPilot 会做什么 | 你来决定 |
| --- | --- | --- |
| 1. 读取参考图 | 判断图片角色，并提取固定视觉锚点。 | 确认或修正角色不变特征。 |
| 2. 选择素材路线 | 进入表情包/社群素材路线，并考虑平台要求。 | 接受路线，或切换到海报、周边、节日图等类型。 |
| 3. 输出概念草案 | 分开展示文案、场景、图像提示词、负面约束和安全说明。 | 选择、修改或删掉某些概念。 |
| 4. 生成前确认 | 在调用图像生成前展示最终提示词包。 | 确认生成、继续修改，或换一个素材方向。 |
